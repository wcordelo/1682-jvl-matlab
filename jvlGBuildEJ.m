...
filename = 'stolNF.avl'; 
casename = 'Final uSTOL Aircraft'; % file title
...
% Wing parameters
b           = 31.1; % span
cmac        = 4.93; % mean aerodynamic chord
lambda      = 1.0; % taper ratio
nProp       = 10; % total number of propulsors
R           = 1.121; % propeller radius
...
% Tail parameters
Vh          = 0.672; % horizontal tail volume
Vv          = 0.08; %0.4; % vertical tail volume
lRat        = 0.55; % moment arm/wingspan
ARh         = 3.00; % H-tail aspect ratio
...
S = b*cmgc; % total wing planform area
h = pi*R^2*(nProp-1)/b; % jet height
yVec = [0; b/3; b/2]; % spanwise section position
cVec = cr - 2*(cr-ct)/b.*yVec; % section chord
