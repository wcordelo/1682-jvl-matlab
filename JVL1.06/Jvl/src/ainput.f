...
C
     ...
     &     REFLD(ICONX,NWRK),
     &     GAIND(ICONX,NWRK)
      REAL GAING(ICONX,NWRK)
C
      INTEGER IJETTD(ICONX,NFMAX), NSJET(NFMAX)
      REAL ANJET(ICONX,NFMAX), REFLJ(ICONX), GAINJ(ICONX,NFMAX)
C
C
      REAL    RINPUT(10)
      INTEGER IINPUT(10)
...
C
         IF(LDUPL) THEN
          ISURFD = ISURF
          CALL SDUPL(ISURFD,YDUPL,'YDUP')
C
          IF(LJET(ISURF)) THEN
           NVCJ(ISURFD) = NVCJ(ISURF)
           CSPACEJ(ISURFD) = CSPACEJ(ISURF)
C
           LJET(ISURFD) = LJET(ISURF)
           NSJET(ISURFD) = NSJET(ISURF)
           DO ISJET=1, NSJET(ISURF)
             IJETTD(ISJET,ISURFD) = IJETTD(ISJET,ISURF)
             GAINJ(ISJET,ISURFD) = GAINJ(ISJET,ISURF)*REFLJ(ISJET)
             ANJET(ISJET,ISURFD) = ANJET(ISJET,ISURF)*REFLJ(ISJET)
           ENDDO
          ENDIF
         ENDIF
C
...
C
C===========================================================================
      ELSEIF (KEYWD.EQ.'JET ') THEN
C------ mark surface as having jet sheet
        IF(ISURF.EQ.0) THEN
         WRITE(*,9000) '** Misplaced line', ILINE, LINE(1:NLINE)
         WRITE(*,*)    '** No active surface for jet variable'
         GO TO 10
        ENDIF
C
        WRITE(*,*) '    ... with jet sheet'
C
        LJET(ISURF) = .TRUE.
C
        CALL RDLINE(LUN,LINE,NLINE,ILINE)
C
C------ increment jet-variable declaration counter for this surface
        NSJET(ISURF) = NSJET(ISURF) + 1
        ISJET = MIN( NSJET(ISURF) , ICONX )
C
C------ extract jet variable name
        NNAME = INDEX(LINE,' ') - 1
        IF(NNAME.LE.0) THEN
         WRITE(*,*) '** Bad jet declaration line:  ', LINE
         STOP
        ENDIF
C
C------ see if this jet variable has already been declared
        DO N = 1, NVARJET
...
C------ new control variable... assign slot for it
        NVARJET = NVARJET + 1
        IJET = MIN( NVARJET , NJMAX )
        JNAME(IJET) = LINE(1:NNAME)
C
 82     CONTINUE
        IJETTD(ISJET,ISURF) = IJET
C
C------ read numbers after control variable name
        NINPUT = 5
        CALL GETFLT(LINE(NNAME+1:120),RINPUT,NINPUT,ERROR)
        IF(ERROR) THEN
         WRITE(*,*) '*** Bad jet data line:  ', LINE
         STOP
        ENDIF
C
        IF(NINPUT.LT.1) THEN
         GAINJ(ISJET,ISURF) = 1.0
        ELSE
         GAINJ(ISJET,ISURF) = RINPUT(1)
        ENDIF
C
        IF(NINPUT.LT.2) THEN
         REFLJ(ISJET) = 1.0
        ELSE
         REFLJ(ISJET) = RINPUT(2)
        ENDIF
C
        IF(NINPUT.LT.3) THEN
         ANJET(ISJET,ISURF) = 0.0
        ELSE
         ANJET(ISJET,ISURF) = RINPUT(3)
        ENDIF
C
        IF(NINPUT.LT.4 .OR. NVCJ(ISURF).EQ.0) THEN
         NVCJ(ISURF) = NVC
        ELSE
         NVCJ(ISURF) = INT( RINPUT(4) + 0.5 )
        ENDIF
        NVCJ(ISURF) = MAX( 1 , NVCJ(ISURF) )
C
        IF(NINPUT.LT.5) THEN
         CSPACEJ(ISURF) = CSPACE
        ELSE
         CSPACEJ(ISURF) = RINPUT(5)
        ENDIF
...
C---- normal end-of-file exit point
 900  CONTINUE
      CLOSE(UNIT=LUN)
C
C---- set total numbers for solid surfaces
      NVORS = NVOR
      NSTRIPS = NSTRIP
      NSURFS = NSURF
C
C---- create and add jet surfaces, if any were specified
      DO N = 1, NSURFS
        IF(LJET(N)) THEN
          CALL MAKEJET(N,NVCJ(N),CSPACEJ(N),
     &                 IJETTD(1,N),NSJET(N),GAINJ(1,N),ANJET(1,N))
        ENDIF
      ENDDO
C
C*********************************************************************
C
      NJETS = NSURF - NSURFS
      WRITE (*,2018) MACH0,NBODY,NSURFS,NJETS,NSTRIP,NVOR
