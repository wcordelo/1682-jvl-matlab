...
C---- set  m^-1 F,  I^-1 M,  m^-1 (WxP),  I^-1 (WxH)
      DO K = 1, 3
        MIF(K) = MAINV(K,1)*CFT(1)*QS
     &         + MAINV(K,2)*CFT(2)*QS
     &         + MAINV(K,3)*CFT(3)*QS
        RIM(K) = RIINV(K,1)*CMT(1)*QS*BREFD
     &         + RIINV(K,2)*CMT(2)*QS*CREFD
     &         + RIINV(K,3)*CMT(3)*QS*BREFD
     ...
     &         + RIINV(K,3)*WXH(3)
        DO IU = 1, 6
          MIF_U(K,IU) = 
     &           MAINV(K,1)*CFT_U(1,IU)*QS
     &         + MAINV(K,2)*CFT_U(2,IU)*QS
     &         + MAINV(K,3)*CFT_U(3,IU)*QS
          RIM_U(K,IU) = 
     &           RIINV(K,1)*CMT_U(1,IU)*QS*BREFD
     &         + RIINV(K,2)*CMT_U(2,IU)*QS*CREFD
     &         + RIINV(K,3)*CMT_U(3,IU)*QS*BREFD
          PRF_U(K,IU) =
...
C
        DO N = 1, NCONTROL
          MIF_D(K,N) = 
     &           MAINV(K,1)*CFT_D(1,N)*QS
     &         + MAINV(K,2)*CFT_D(2,N)*QS
     &         + MAINV(K,3)*CFT_D(3,N)*QS
          RIM_D(K,N) = 
     &           RIINV(K,1)*CMT_D(1,N)*QS*BREFD
     &         + RIINV(K,2)*CMT_D(2,N)*QS*CREFD
     &         + RIINV(K,3)*CMT_D(3,N)*QS*BREFD
        ENDDO
C
...
C---- x-acceleration
      IEQ = JEU
      ASYS(IEQ,JEU) = -CFT_U(1,1)*QS /RMASS / VEE
      ASYS(IEQ,JEW) = -CFT_U(1,3)*QS /RMASS / VEE
      ASYS(IEQ,JEQ) =  CFT_U(1,5)*QS /RMASS / ROT  +  VINF(3)*VEE
      ASYS(IEQ,JETH)=  GEE
      DO N = 1, NCONTROL
        BSYS(IEQ,N) =  CFT_D(1,N)*QS /RMASS
      ENDDO
C
C---- z-acceleration
      IEQ = JEW
      ASYS(IEQ,JEU) = -CFT_U(3,1)*QS /RMASS / VEE
      ASYS(IEQ,JEW) = -CFT_U(3,3)*QS /RMASS / VEE
      ASYS(IEQ,JEQ) =  CFT_U(3,5)*QS /RMASS / ROT  -  VINF(1)*VEE
      DO N = 1, NCONTROL
        BSYS(IEQ,N) =  CFT_D(3,N)*QS /RMASS
      ENDDO
C
C---- y-ang.accel.
      IEQ = JEQ
      ASYS(IEQ,JEU) = -CMT_U(2,1)*QSC/RINYY / VEE
      ASYS(IEQ,JEW) = -CMT_U(2,3)*QSC/RINYY / VEE
      ASYS(IEQ,JEQ) =  CMT_U(2,5)*QSC/RINYY / ROT
      DO N = 1, NCONTROL
        BSYS(IEQ,N) =  CMT_D(2,N)*QSC/RINYY
      ENDDO
C
C---- theta rate
      IEQ = JETH
...
C---- y-acceleration
      IEQ = JEV
      ASYS(IEQ,JEV) = -CFT_U(2,2)*QS /RMASS / VEE
      ASYS(IEQ,JEP) =  CFT_U(2,4)*QS /RMASS / ROT  -  VINF(3)*VEE
      ASYS(IEQ,JER) =  CFT_U(2,6)*QS /RMASS / ROT  +  VINF(1)*VEE
      ASYS(IEQ,JEPH)=  GEE
      DO N = 1, NCONTROL
        BSYS(IEQ,N) =  CFT_D(2,N)*QS /RMASS
      ENDDO
C
C---- x-ang.accel.
      IEQ = JEP
      ASYS(IEQ,JEV) = -CMT_U(1,2)*QSB/RINXX / VEE
      ASYS(IEQ,JEP) =  CMT_U(1,4)*QSB/RINXX / ROT
      ASYS(IEQ,JER) =  CMT_U(1,6)*QSB/RINXX / ROT
      DO N = 1, NCONTROL
        BSYS(IEQ,N) =  CMT_D(1,N)*QSB/RINXX
      ENDDO
C
C---- z-ang.accel.
      IEQ = JER
      ASYS(IEQ,JEV) = -CMT_U(3,2)*QSB/RINZZ / VEE
      ASYS(IEQ,JEP) =  CMT_U(3,4)*QSB/RINZZ / ROT
      ASYS(IEQ,JER) =  CMT_U(3,6)*QSB/RINZZ / ROT
      DO N = 1, NCONTROL
        BSYS(IEQ,N) =  CMT_D(3,N)*QSB/RINZZ
      ENDDO
C
C---- phi rate
      IEQ = JEPH
