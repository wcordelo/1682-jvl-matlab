...
C----- new Mach number invalidates close to everything that's stored
      ...
      ENDIF
C
C---- set control and jet variables if they are explicitly specified
      DO IV = 1, NVTOT
        IC = ICON(IV,IR)
C
        DO N = 1, NCONTROL
...
C----- set GAM_J
ccc       WRITE(*,*) ' Solving for vortex  jet-var sensitivities...'
       CALL GJCALC(NVARJET ,GJSTRP,GAM_J)
      ENDIF
C
...
C------------------------------------
          ELSEIF(IC.EQ.ICCL  ) THEN
           VRES(IV) = CLT - CONVAL(IC,IR)*SREF
           VSYS(IV,IVALFA) = CLT_U(1)*VINF_A(1)
     &                     + CLT_U(2)*VINF_A(2)
     &                     + CLT_U(3)*VINF_A(3) + CLT_A
           VSYS(IV,IVBETA) = CLT_U(1)*VINF_B(1)
     &                     + CLT_U(2)*VINF_B(2)
     &                     + CLT_U(3)*VINF_B(3)
           VSYS(IV,IVROTX) = CLT_U(4)
           VSYS(IV,IVROTY) = CLT_U(5)
           VSYS(IV,IVROTZ) = CLT_U(6)
C
           DO N = 1, NCONTROL
             NV = IVTOT + N
             VSYS(IV,NV) = CLT_D(N)
...
C------------------------------------
          ELSEIF(IC.EQ.ICCY  ) THEN
           VRES(IV) = CYT - CONVAL(IC,IR)*SREF
           VSYS(IV,IVALFA) = CYT_U(1)*VINF_A(1)
     &                     + CYT_U(2)*VINF_A(2)
     &                     + CYT_U(3)*VINF_A(3) + CYT_A
           VSYS(IV,IVBETA) = CYT_U(1)*VINF_B(1)
     &                     + CYT_U(2)*VINF_B(2)
     &                     + CYT_U(3)*VINF_B(3)
           VSYS(IV,IVROTX) = CYT_U(4)
           VSYS(IV,IVROTY) = CYT_U(5)
           VSYS(IV,IVROTZ) = CYT_U(6)
C
           DO N = 1, NCONTROL
             NV = IVTOT + N
             VSYS(IV,NV) = CYT_D(N)
...
C------------------------------------
          ELSEIF(IC.EQ.ICMOMX) THEN
           VRES(IV) = (CMT(1)*CA + CMT(3)*SA)*DIR
     &              - CONVAL(IC,IR)*SREF*BREF
           VSYS(IV,IVALFA) = ( CMT_U(1,1)*VINF_A(1)
     &                        +CMT_U(1,2)*VINF_A(2)
     &                        +CMT_U(1,3)*VINF_A(3))*CA*DIR
     &                     + ( CMT_U(3,1)*VINF_A(1)
     &                        +CMT_U(3,2)*VINF_A(2)
     &                        +CMT_U(3,3)*VINF_A(3))*SA*DIR
     &                     + (CMT(1)*CA_A + CMT(3)*SA_A)*DIR
           VSYS(IV,IVBETA) = ( CMT_U(1,1)*VINF_B(1)
     &                        +CMT_U(1,2)*VINF_B(2)
     &                        +CMT_U(1,3)*VINF_B(3))*CA*DIR
     &                     + ( CMT_U(3,1)*VINF_B(1)
     &                        +CMT_U(3,2)*VINF_B(2)
     &                        +CMT_U(3,3)*VINF_B(3))*SA*DIR
           VSYS(IV,IVROTX) = (CMT_U(1,4)*CA + CMT_U(3,4)*SA)*DIR
           VSYS(IV,IVROTY) = (CMT_U(1,5)*CA + CMT_U(3,5)*SA)*DIR
           VSYS(IV,IVROTZ) = (CMT_U(1,6)*CA + CMT_U(3,6)*SA)*DIR
C
           DO N = 1, NCONTROL
             NV = IVTOT + N
             VSYS(IV,NV) = (CMT_D(1,N)*CA + CMT_D(3,N)*SA)*DIR
           ENDDO
C
           DO N = 1, NVARJET
             NV = IVTOT + NCONTROL + N
             VSYS(IV,NV) = (CMT_J(1,N)*CA + CMT_J(3,N)*SA)*DIR
           ENDDO
C
C------------------------------------
          ELSEIF(IC.EQ.ICMOMY) THEN
           VRES(IV) = CMT(2)
     &              - CONVAL(IC,IR)*SREF*CREF
           VSYS(IV,IVALFA) = CMT_U(2,1)*VINF_A(1)
     &                     + CMT_U(2,2)*VINF_A(2)
     &                     + CMT_U(2,3)*VINF_A(3)
           VSYS(IV,IVBETA) = CMT_U(2,1)*VINF_B(1)
     &                     + CMT_U(2,2)*VINF_B(2)
     &                     + CMT_U(2,3)*VINF_B(3)
           VSYS(IV,IVROTX) = CMT_U(2,4)
           VSYS(IV,IVROTY) = CMT_U(2,5)
           VSYS(IV,IVROTZ) = CMT_U(2,6)
C
           DO N = 1, NCONTROL
             NV = IVTOT + N
             VSYS(IV,NV) = CMT_D(2,N)
           ENDDO
C
           DO N = 1, NVARJET
             NV = IVTOT + NCONTROL + N
             VSYS(IV,NV) = CMT_J(2,N)
           ENDDO
C
C------------------------------------
          ELSEIF(IC.EQ.ICMOMZ) THEN
           VRES(IV) = (CMT(3)*CA - CMT(1)*SA)*DIR
     &              - CONVAL(IC,IR)*SREF*BREF
           VSYS(IV,IVALFA) = ( CMT_U(3,1)*VINF_A(1)
     &                        +CMT_U(3,2)*VINF_A(2)
     &                        +CMT_U(3,3)*VINF_A(3))*CA*DIR
     &                     - ( CMT_U(1,1)*VINF_A(1)
     &                        +CMT_U(1,2)*VINF_A(2)
     &                        +CMT_U(1,3)*VINF_A(3))*SA*DIR
     &                     + (CMT(3)*CA_A - CMT(1)*SA_A)*DIR
           VSYS(IV,IVBETA) = ( CMT_U(3,1)*VINF_B(1)
     &                        +CMT_U(3,2)*VINF_B(2)
     &                        +CMT_U(3,3)*VINF_B(3))*CA*DIR
     &                     - ( CMT_U(1,1)*VINF_B(1)
     &                        +CMT_U(1,2)*VINF_B(2)
     &                        +CMT_U(1,3)*VINF_B(3))*SA*DIR
           VSYS(IV,IVROTX) = (CMT_U(3,4)*CA - CMT_U(1,4)*SA)*DIR
           VSYS(IV,IVROTY) = (CMT_U(3,5)*CA - CMT_U(1,5)*SA)*DIR
           VSYS(IV,IVROTZ) = (CMT_U(3,6)*CA - CMT_U(1,6)*SA)*DIR
C
           DO N = 1, NCONTROL
             NV = IVTOT + N
             VSYS(IV,NV) = (CMT_D(3,N)*CA - CMT_D(1,N)*SA)*DIR
           ENDDO
C
           DO N = 1, NVARJET
             NV = IVTOT + NCONTROL + N
             VSYS(IV,NV) = (CMT_J(3,N)*CA - CMT_J(1,N)*SA)*DIR
           ENDDO
C
C------------------------------------
          ELSEIF(IC.EQ.ICJETJ) THEN
           VRES(IV) = CJT - CONVAL(IC,IR)*SREF
           DO N = 1, NVARJET
             NV = IVTOT + NCONTROL + N
             VSYS(IV,NV) = CJT_J(N)
           ENDDO
...
C------------------------------------
          ELSEIF(IC.EQ.ICJETT) THEN
           VRES(IV) = CJT - 2.0*CQT - CONVAL(IC,IR)*SREF
           DO N = 1, NVARJET
             NV = IVTOT + NCONTROL + N
             VSYS(IV,NV) = CJT_J(N) - 2.0*CQT_J(N)
           ENDDO
C
C------------------------------------
          ELSE
...
C------ set Newton deltas
        ...
        DO N = 1, NVARJET
          ...
          DDJ(N) = -VRES(IV)
C
C-------- new jet momentum invalidates W.n matrix AICN, and unit-freestream GAM_U
          IF(ABS(DDJ(N)) .GT. EPSJ) THEN
           LAIC = .FALSE.
           LGMU = .FALSE.
c           LSRD = .FALSE.
           LSOL = .FALSE.
...
C------- display Newton deltas
 ...
 1902     FORMAT(1X,A4,5A11,1X, 30(A3,A8,A1) )
         ENDIF
         WRITE(*,1905) ITER, 
     &                 DAL/DTR, DBE/DTR, 
     ...
     &                 (DDC(K), K=1, NCONTROL),
     &                 (DDJ(K), K=1, NVARJET)
 1905    FORMAT(1X,I3, 5E11.3, 30E12.3)
        ENDIF
C
...
C
        ...
        ENDIF
C
        IF(ABS(WROT(1)+DWX)*BREF .GT. DMAXR) THEN
         WRITE(*,*) 'Cannot trim.  Roll rate too large.  pb/2V =', 
     &               (WROT(1)+DWX)*BREF*0.5
         RETURN
        ENDIF
C
        IF(ABS(WROT(2)+DWY)*CREF .GT. DMAXR) THEN
         WRITE(*,*) 'Cannot trim.  Pitch rate too large.  qc/2V =',
     &               (WROT(2)+DWY)*CREF*0.5
         RETURN
        ENDIF
C
        IF(ABS(WROT(3)+DWZ)*BREF .GT. DMAXR) THEN
...
C------ update
        ...
        WROT(3) = WROT(3) + DWZ
        DO N = 1, NCONTROL
          DELCON(N) = DELCON(N) + DDC(N)
        ENDDO
        DO N = 1, NVARJET
          DELJET(N) = DELJET(N) + DDJ(N)
        ENDDO
C
C
C------ set VINF() vector from new ALFA,BETA
        CALL VINFAB
C
C------ need to recalculate AIC matrix and WC,WV matrices if there's a jet
        CALL SETUP
C
...
C
     ...
     &       + WC(2,I)*ENC(2,I)
     &       + WC(3,I)*ENC(3,I)
     &       + (VINF(1)+VROT(1))*ENC(1,I)
     &       + (VINF(2)+VROT(2))*ENC(2,I)
     &       + (VINF(3)+VROT(3))*ENC(3,I)
...
C----- go over jet surfaces
       DO I = NVORS+1, NVOR
C-------- jet curvature/loading
C
C          DPjet * Delta(w).n - Gamma  =  0
C
         ISTRIP = ISTRPV(I)
         DPJET = 0.0
         DO N = 1, NVARJET
...
C
         IM = IJETM(I)
         RES = DPJET * (  (WC(1,I)-WC(1,IM))*ENC(1,I)
     &                  + (WC(2,I)-WC(2,IM))*ENC(2,I)
     &                  + (WC(3,I)-WC(3,IM))*ENC(3,I) )  -  GAM(I)
         WRITE(*,*) -I, RES, DPJET
