...
C
      ...
      REAL SLE(NSMAX)
      LOGICAL LCOLHC
      CHARACTER*24 COL1, COL2, COL3, COL4, COL5
C
      REAL CL_TOT(NSMAX), CL_CIRC(NSMAX), CL_PERP(NSMAX), CNC(NSMAX)
C
      REAL RINP(3)
      LOGICAL ERROR
C
...
C---- Find the min/max of Y,CL,CNC,downwash
 50   YMIN =  RV1(2,1)
      ...
      DO J = 1, NSTRIPS
        ...
        FMIN = MIN( FMIN, CNC(J), 0.0 )
        FMAX = MAX( FMAX, CNC(J), 0.0 )
...
C--- L8 norm
        WMIN = WMIN + MIN( -DWWAKE(J)  , 0.0 ) ** 8
        WMAX = WMAX + MAX( -DWWAKE(J)  , 0.0 ) ** 8
C--- Regular min/max
        CMIN = MIN( CMIN, CL_PERP(J), CL_TOT(J) , 0.0 )
        CMAX = MAX( CMAX, CL_PERP(J), CL_TOT(J) , 0.0 )
c        WMIN = MIN( WMIN, -DWWAKE(J)  , 0.0 )
c        WMAX = MAX( WMAX, -DWWAKE(J)  , 0.0 )
      END DO
C
C--- Process L8 sums to norms
      IF(WMIN .NE. 0.0) WMIN = -(ABS(WMIN)/NSTRIPS) ** 0.125
      IF(WMAX .NE. 0.0) WMAX =  (ABS(WMAX)/NSTRIPS) ** 0.125
C
...
C------ plot cl_circ
        CALL NEWCOLORNAME(COL4)
        CALL XYLINE(NJ(N),SLE(J),CL_CIRC(J),SMIN,SSF,0.0,CSF,2)
        IF(LLABSURF) THEN
          XLAB = SMOD(SLE(JLABCL))         
          YLAB = CSF*CL_CIRC(JLABCL)         
          CALL PLNUMB(XLAB+0.5*CH,YLAB+0.5*CH,0.8*CH,FLOAT(N),0.0,-1)
        ENDIF
C
C------ plot cl_perp
...
C
      XTMP(1) = 0.75*SMAX 
      XTMP(2) = 0.85*SMAX
      YLAB = MAX(WSF*WMAX,CSF*CMAX) + 14.25*CH
C
      IF(LCLPERPLT) THEN
       CALL NEWCOLORNAME(COL3)
...
C
      DXL = 2.2*CH
      CALL PLCHAR(SMOD(XTMP(2))+1.5*CH+DXL,CSF*YTMP(2)-0.5*CH,
     &           1.2*CH,'+c'  ,0., 2)
      DXL = DXL +1.2*CH
      CALL PLMATH(SMOD(XTMP(2))+2.5*CH+DXL,CSF*YTMP(2)-0.9*CH,
     &           0.8*CH, 'V' ,0., 1)
      CALL PLCHAR(SMOD(XTMP(2))+3.2*CH+DXL,CSF*YTMP(2)-1.1*CH,
     &           0.7*CH,'j'  ,0., 1)
C
      CALL NEWCOLORNAME(COL4)
      YTMP(1) = YLAB/CSF - 2.2*CH/CSF
...
C
      ...
      CALL PLCHAR(XLAB2,YLAB2,0.8*CH,'JVL ',0.0,4)
...
C---- Case title
      XLAB = SMOD(SMIN)
      YLAB = MAX(WSF*WMAX,CSF*CMAX) + 20.2*CH + 2.2*CH*FLOAT(NCONLIN)
C
      CALL NEWPEN(3)
...
C--- Flow condition and forces
      RX = DIR*WROT(1)*BREF/2.0
      RY =     WROT(2)*CREF/2.0
      RZ = DIR*WROT(3)*BREF/2.0
C
      CL = CLT/SREF
      CY = CYT/SREF
      CD = CDT/SREF
C
      CR = DIR*CMT(1)/(SREF*BREF)
      CM =     CMT(2)/(SREF*CREF)
      CN = DIR*CMT(3)/(SREF*BREF)
C
      CDI = CDFFI/SREF
      CDV = CDFFV/SREF
C
...
C
      ...
      CALL PLNUMB(XL2+8.0*CH,YLAB,CH, RX       ,0.0,4)
C
      CALL PLCHAR(XL3       ,YLAB,CH,'  CL = ',0.0,7)
      CALL PLNUMB(XL3+7.0*CH,YLAB,CH, CL      ,0.0,4)
C
      CALL PLCHAR(XL4       ,YLAB,CH,'  Cl = ',0.0,7)
      CALL PLNUMB(XL4+7.0*CH,YLAB,CH, CR      ,0.0,4)
C
C
      YLAB = YLAB - 2.2*CH
...
C
      ...
      CALL PLNUMB(XL3+7.0*CH,YLAB,CH, CY      ,0.0,4)
C
      CALL PLCHAR(XL4       ,YLAB,CH,'  Cm = ',0.0,7)
      CALL PLNUMB(XL4+7.0*CH,YLAB,CH, CM      ,0.0,4)
C
C
      YLAB = YLAB - 2.2*CH
...
C
      ...
      CALL PLNUMB(XL2+8.0*CH,YLAB,CH, RZ       ,0.0,4)
C
      CALL PLCHAR(XL3       ,YLAB,CH,'  CD = ',0.0,7)
      CALL PLNUMB(XL3+7.0*CH,YLAB,CH, CD      ,0.0,5)
C
      CALL PLCHAR(XL4       ,YLAB,CH,'  Cn = ',0.0,7)
      CALL PLNUMB(XL4+7.0*CH,YLAB,CH, CN     , 0.0,4)
C
C
      YLABI = YLAB
      YLABI = YLABI - 2.2*CH
     ...
     &                         0.8*CH,'i'      ,0.0,1)
      CALL PLNUMB(XL3+7.0*CH,YLABI,CH, CDI     ,0.0,5)
C
...
C
      ...
      CALL PLCHAR(XL3+3.8*CH,YLABI-0.4*CH,
     &                         0.8*CH,'p'      ,0.0,1)
      CALL PLNUMB(XL3+7.0*CH,YLABI,CH, CDV     ,0.0,5)
C
      IF(NVARJET .GT. 0) THEN
       YLABI = YLABI - 2.2*CH
       CALL PLCHAR(XL3       ,YLABI,CH,'  CJ = ',0.0,7)
       CALL PLNUMB(XL3+7.0*CH,YLABI,CH, CJ      ,0.0,4)
C
       CALL PLCHAR(XL4       ,YLABI,CH,' e   = ',0.0,7)
       CALL PLCHAR(XL4+1.9*CH,YLABI-0.4*CH,0.7*CH,'vec',0.0,3)
       CALL PLNUMB(XL4+7.0*CH,YLABI,CH, SPANEV  ,0.0,4)
C
C
       YLABI = YLABI - 2.2*CH
       CALL PLCHAR(XL3       ,YLABI,CH,'  CQ = ',0.0,7)
       CALL PLNUMB(XL3+7.0*CH,YLABI,CH, CQ      ,0.0,4)
      ENDIF
C
C
      YLAB = YLAB - 0.3*CH
