...
C
        IF(ABS(RL(2,NLNODE)) .GT. 0.0001*BREF) THEN
...
C
      ...
      SUBROUTINE MAKEJET(ISURF,NVC1,CSPACE,
     &                   IJETTD,NSJET,GAINJ,ANJET)
C--------------------------------------------------------------
C     Sets up jet surface trailing from lifting surface ISURF.
C     NVJ is the number of bound vortices.
C--------------------------------------------------------------
      INCLUDE 'AVL.INC'
C
      INTEGER IJETTD(*)
...
C
      IF(NVC.GT.KCMAX) THEN
       WRITE(*,*) '* MAKEJET: Array overflow.  Increase KCMAX to', NVC
       NVC = KCMAX
      ENDIF
C
C---- index of new jet surface
      NN = NSURF + 1
      IF(NN.GT.NFMAX) THEN
       WRITE(*,*) 'MAKEJET: Surface array overflow. Increase NFMAX.'
       STOP
      ENDIF
C
C---- jet surface
      IFTYPE(NN) = 1
C
C---- jet has same logical index as the physical surface
      LSURF(NN) = LSURF(ISURF)
C
C====================================================
C---- define strips trailing from ISURF surface's strips
C
      IFRST(NN) = NVOR   + 1     ! index of first element
      JFRST(NN) = NSTRIP + 1     ! index of first chord strip
      NK(NN) = NVC               ! number of elements along chord
      NJ(NN) = NJ(ISURF)         ! number of strips along span
C
      NSPAN = NJ(NN)
C
C------ go over chord strips
        DO 150 ISPAN = 1, NSPAN
C
C-------- index of strip on lifting surface
          ISF = JFRST(ISURF) + ISPAN - 1
C
C-------- store jet strip in global data arrays
          NSTRIP = NSTRIP + 1
C
          RLE1(1,NSTRIP) = RLE1(1,ISF) + CHORD1(ISF)
          RLE1(2,NSTRIP) = RLE1(2,ISF)
          RLE1(3,NSTRIP) = RLE1(3,ISF)
          CHORD1(NSTRIP) = CHORD1(ISF)
C
          RLE2(1,NSTRIP) = RLE2(1,ISF) + CHORD2(ISF)
          RLE2(2,NSTRIP) = RLE2(2,ISF)
          RLE2(3,NSTRIP) = RLE2(3,ISF)
          CHORD2(NSTRIP) = CHORD2(ISF)
C
          RLE(1,NSTRIP) = RLE(1,ISF) + CHORD(ISF)
          RLE(2,NSTRIP) = RLE(2,ISF)
          RLE(3,NSTRIP) = RLE(3,ISF)
          CHORD(NSTRIP) = CHORD(ISF)
C
          WSTRIP(NSTRIP) = WSTRIP(ISF)
          TANLE(NSTRIP)  = TANTE(ISF)
          TANTE(NSTRIP)  = 0.
          AINC(NSTRIP) = 0.
C
          LVISCSTRP(NSTRIP) = .FALSE.
C
C
          IJFRST(NSTRIP) = NVOR + 1
          NVSTRP(NSTRIP) = NVC
...
C
C-------- set chordwise spacing fraction arrays
ccc       CALL CSPACER(NVC,CSPACE,CLAFC, XPT,XVR,XSR,XCP)
          CALL CSPACER(NVC,CSPACE,CLAFC, XPT,XCP,XSR,XVR)
          XPT(NVC+1) = 1.0
          XCP(NVC+1) = 1.0
          XSR(NVC+1) = 1.0
C
c          XVRL =     (COS(PI*1.0/(2.0*NK(ISURF)+1.0)) - 1.0)
c     &         - 0.5*(COS(PI*2.0/(2.0*NK(ISURF)+1.0)) - 1.0)
c          DXX = XVRL - XVR(1)
c          DO IVC = 1, NVC
c            XPT(IVC) = XPT(IVC) + DXX
c            XVR(IVC) = XVR(IVC) + DXX
c            XSR(IVC) = XSR(IVC) + DXX
c            XCP(IVC) = XCP(IVC) + DXX
c          ENDDO



          DXCS = 0.5*(1.0 - COS(PI*1.0/(2.0*NK(ISURF)+1.0)))
          DXCW = 0.5*(1.0 - COS(PI*1.0/(2.0*NK(NN   )+1.0)))
C
          DX1 = CHORD1(ISF)*DXCS + CHORD1(NSTRIP)*DXCW
          DX2 = CHORD2(ISF)*DXCS + CHORD2(NSTRIP)*DXCW
          DXA = CHORD(ISF) *DXCS + CHORD(NSTRIP) *DXCW
C
          WLEN = 2.0*BREF
          FAC1 = 1.0 / (1.0 + CHORD1(NSTRIP)/WLEN)
          FAC2 = 1.0 / (1.0 + CHORD2(NSTRIP)/WLEN)
          FAC  = 1.0 / (1.0 + CHORD(NSTRIP) /WLEN)
C
C-------- go over vortices in this strip
          DO 1505 IVC = 1, NVC
            NVOR = NVOR + 1
C
            RV1(1,NVOR) = RLE1(1,NSTRIP) - DX1
     &                  + XVR(IVC)*CHORD1(NSTRIP) / (1.0-FAC1*XVR(IVC))
            RV1(2,NVOR) = RLE1(2,NSTRIP)
            RV1(3,NVOR) = RLE1(3,NSTRIP)
C
            RV2(1,NVOR) = RLE2(1,NSTRIP) - DX2
     &                  + XVR(IVC)*CHORD2(NSTRIP) / (1.0-FAC2*XVR(IVC))
            RV2(2,NVOR) = RLE2(2,NSTRIP)
            RV2(3,NVOR) = RLE2(3,NSTRIP)
C
            RV(1,NVOR) = RLE(1,NSTRIP) - DXA
     &                 + XVR(IVC)*CHORD(NSTRIP) / (1.0-FAC*XVR(IVC))
            RV(2,NVOR) = RLE(2,NSTRIP)
            RV(3,NVOR) = RLE(3,NSTRIP)
C
            RC(1,NVOR) = RLE(1,NSTRIP) - DXA
     &                 + XCP(IVC+1)*CHORD(NSTRIP) / (1.0-FAC*XCP(IVC+1))
            RC(2,NVOR) = RLE(2,NSTRIP)
            RC(3,NVOR) = RLE(3,NSTRIP)
C
            RS(1,NVOR) = RLE(1,NSTRIP) - DXA
     &                 + XSR(IVC+1)*CHORD(NSTRIP) / (1.0-FAC*XSR(IVC+1))
            RS(2,NVOR) = RLE(2,NSTRIP)
            RS(3,NVOR) = RLE(3,NSTRIP)


c            if(nstrip.eq.nstrips+1) then
c            write(*,*) ivc, 1.0 + XVR(IVC)/(1.0-FAC*XVR(IVC)), 
c     &                      1.0 + XCP(IVC)/(1.0-FAC*XCP(IVC))
c            endif
C
            SLOPEC(NVOR) = 0.0
            SLOPEV(NVOR) = 0.0
C
            DXOC = XPT(IVC+1) / (1.0-FAC*XPT(IVC+1))
     &           - XPT(IVC  ) / (1.0-FAC*XPT(IVC  ))
            DXV(NVOR) = DXOC*CHORD(NSTRIP)
            CHORDV(NVOR) = CHORD(NSTRIP)
            ISURFV(NVOR) = NN
            LSURFV(NVOR) = LSURF(NN)
            ISTRPV(NVOR) = NSTRIP
C
            IF(IVC.EQ.1) THEN
             IJETM(NVOR) = IFRST(ISURF)-1 + NK(ISURF)*ISPAN
            ELSE
             IJETM(NVOR) = NVOR-1
            ENDIF
 1505     CONTINUE
C
          DO ISJET = 1, NSJET
C---------- set influence of jet variable on jet momentum
C-            for this strip, and for the total
            N = IJETTD(ISJET)
            WJSPAN(N) = WJSPAN(N) + WSTRIP(NSTRIP)
            GJSTRP(NSTRIP,N) = GAINJ(ISJET)
            HJSTRP(NSTRIP,N) = ANJET(ISJET)
          ENDDO
C
 150    CONTINUE
C
