...
C
 ...
 2020 FORMAT(
     ...
     &  /'  HI de lines ',L2,7X,'VV ort velocity ',L2 )
...
C----- Display options 
 50    CONTINUE
       ...
       LCOLHC = IDEVH.EQ.4
       WRITE(*,5500) ROB, SIZE, CPFAC, ENFAC, WSFAC, LCOLHC,
...
C      IF(.NOT.LPLOT) CALL COLORMAP(NCOLORS)
      ...
      XPLT = PMARG
      YPLT = PMARG
      CALL PLCHAR(XPLT,YPLT,0.6*SCH,'JVL ',0.0,4)
...
C------ Chordlines plotted
        IF(LCHORDLINE .AND. IFTYPE(N).EQ.0) THEN
         IP = 0
         DO J = J1, JN, JINC
           IP = IP+1
...
C------ Strip camberline plotted (approx., integrated from camber slopes)
        IF(LCAMBER .AND. IFTYPE(N).EQ.0) THEN
         CALL GETPEN(IPEN)
         CALL GETCOLOR(ICOL)
C
         DO IPASS = 1, 2
...
C------ Strip loading plotted
        IF((LLOADPLT .AND. IFTYPE(N).EQ.0) .OR.
     &     (LWAKLPLT .AND. IFTYPE(N).EQ.1)      ) THEN
         CALL GETPEN(IPEN)
         CALL GETCOLOR(ICOL)
C
C------- Plot the load vectors (up from surface)
c         IF(IFTYPE(N).EQ.0) THEN
          CALL NEWPEN(5)
          CALL NEWCOLORNAME('GREEN')
c         ELSE
c          CALL NEWPEN(4)
c          CALL NEWCOLORRGB(0,180,0)
c         ENDIF 
C
         IP = 0
         CPSCL = CPFAC*CREF
...
C
             DO K = 1, 3
               VC(K) = VINF(K)
     &               + VROT(K)
     &               + WCSRD_U(K,IC,1)*VINF(1)
     &               + WCSRD_U(K,IC,2)*VINF(2)
     &               + WCSRD_U(K,IC,3)*VINF(3)
     &               + WCSRD_U(K,IC,4)*WROT(1)
     &               + WCSRD_U(K,IC,5)*WROT(2)
     &               + WCSRD_U(K,IC,6)*WROT(3)
     &               + WC(K,IC)
             ENDDO
C
             IP = IP+1
             ...
             PTS_LINES(1,2,IP) = RC(1,IC) + WSCL*VC(1)
             PTS_LINES(2,2,IP) = RC(2,IC) + WSCL*VC(2)
             PTS_LINES(3,2,IP) = RC(3,IC) + WSCL*VC(3)
             ID_LINES(IP) = 0
           END DO
         END DO
C
...
C
             DO K = 1, 3
               VV(K) = VINF(K)
     &               + VROT(K)
     &               + WVSRD_U(K,IV,1)*VINF(1)
     &               + WVSRD_U(K,IV,2)*VINF(2)
     &               + WVSRD_U(K,IV,3)*VINF(3)
     &               + WVSRD_U(K,IV,4)*WROT(1)
     &               + WVSRD_U(K,IV,5)*WROT(2)
     &               + WVSRD_U(K,IV,6)*WROT(3)
     &               + WV(K,IV)
             ENDDO
C
             IP = IP+1
             ...
             PTS_LINES(1,2,IP) = RV(1,IV) + WSCL*VV(1)
             PTS_LINES(2,2,IP) = RV(2,IV) + WSCL*VV(2)
             PTS_LINES(3,2,IP) = RV(3,IV) + WSCL*VV(3)
             ID_LINES(IP) = 0
           END DO
         END DO
C
...
C
C- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        IF(IFTYPE(N).EQ.0) THEN
C
C------- Surface boundary lines
         IP = 0
         IP = IP+1
